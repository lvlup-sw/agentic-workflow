...
internal sealed class ForkJoinHandlerEmitter
{
        ...
        ForkPathModel path)
    {
        ...
        sb.AppendLine($"        Fork_{sanitizedId}_Path{path.PathIndex}Status = Agentic.Workflow.Definitions.ForkPathStatus.Success;");
        sb.AppendLine();
        ...
        ForkModel fork)
    {
        ...
        for (var i = 0; i < fork.Paths.Count; i++)
        {
            ...
            sb.Append($"(Fork_{sanitizedId}_Path{i}Status is Agentic.Workflow.Definitions.ForkPathStatus.Success ");
            sb.Append($"or Agentic.Workflow.Definitions.ForkPathStatus.Failed ");
            sb.Append($"or Agentic.Workflow.Definitions.ForkPathStatus.FailedWithRecovery)");
        }
